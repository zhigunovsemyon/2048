...
{ // Создание сообщения
	...
	SDL_asprintf(&message, "%s\n%s\n%s\n%s%s%s\n%s%s\n%s%s\n%s%hhu\n%s\n",
				 "Автор игры: Gabriele Cirulli", "Автор данной реализации: Жигунов С. Н.",
				 "Добро пожаловать в игру 2048!", // Заголовок
				 "Включён ", (Params->Flags & FLAG_DARKMODE) ? "тёмный" : "светлый",
				 " режим", // Цветовой режим
				 "V-Sync ", (Params->Flags & FLAG_VSYNC) ? "включен" : "отключен", // v-sync
				 "Используется управление ",
				 (Params->Flags & FLAG_WASDKEY) ? "WASD" // схема управления wasd
				 : (Params->Flags & FLAG_VIKEY) ? "vi"
												: "стрелками", // либо vi, либо стрелки
	...
	SDL_Texture *greet = CreateMessageTexture(rend, &Assets->cols[COL_FG], &Assets->cols[COL_BG],
											  &txt_size, FONT, message, SDL_FALSE);
	if (!greet)
	{
	...
	while (SDL_TRUE)
	{
		...
		if (CheckForResize(window, Params, &ev, WIN_MIN))
		{
			...
			txt_size.w = Params->WinSize.x, txt_size.h = Params->WinSize.y;
			greet = CreateMessageTexture(rend, &Assets->cols[COL_FG], &Assets->cols[COL_BG],
										 &txt_size, FONT, message, SDL_FALSE);
			if (!greet)
			{
				SDL_SetError("ошибка создания текстуры приветствия!");
		...
		case SDL_KEYUP: // Если была нажата любая клавиша
			SDL_DestroyTexture(greet);
			// Если нажата клавиша Q -- осуществляется выход из игры, в противном случае -- начало
			// игры
